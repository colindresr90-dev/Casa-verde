      ...
      <div className="hidden md:flex items-center gap-1 bg-primary rounded-full px-2 py-2 shadow-lg">
        ...
        {navItems.map((item) => (
          <Link
            ...
            to={item.href}
            className={cn(
              "px-4 py-2 rounded-full text-sm font-medium text-primary-foreground hover:shadow-[0_0_12px_hsl(var(--accent)/0.6)] transition-all duration-200 text-roll",
              location.pathname === item.href && "bg-accent text-accent-foreground"
            )}
          >
            <span className="text-roll-inner"><span>{item.label}</span><span>{item.label}</span></span>
          </Link>
        ))}
      </div>
          ...
          <div className="absolute top-14 left-1/2 -translate-x-1/2 bg-primary rounded-2xl px-6 py-4 shadow-xl flex flex-col gap-2 min-w-[200px]">
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  "px-4 py-2 rounded-full text-sm font-medium text-primary-foreground hover:shadow-[0_0_12px_hsl(var(--accent)/0.6)] transition-all text-center text-roll",
                  location.pathname === item.href && "bg-accent text-accent-foreground"
                )}
              >
                <span className="text-roll-inner"><span>{item.label}</span><span>{item.label}</span></span>
